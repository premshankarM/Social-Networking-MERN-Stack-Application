import { useContext, useEffect, useState } from "react";
import { useParams } from "react-router";
import { UserContext } from "../../App";
import appService from "../../services/app.service";

const UserProfile = () => {
    const [user,setUser] = useState({})
    const [posts,setPosts] = useState([])
    const {state,dispath} = useContext(UserContext);
    const {id} = useParams()
    useEffect(()=>{
        appService.getOtherProfileDetails({_id:id}).then((result) => {
            console.log(result);
            setUser(result.user);
            setPosts(result.posts)
        }).catch((err) => {
            
        });
    },[])
    return (
        <div>
        <div class="container mx-auto profile-container pt-5">
        <div class="row border-bottom pb-5">
            <div class="col-sm-4 col-md-4 col-lg-4 col-xs-12" >
                <img src='https://cdn.pixabay.com/photo/2017/09/20/12/12/girl-2768346__340.jpg' className=' rounded-circle d-block m-auto user-profile-image'  />
            </div>
            <div class="col-sm-8 col-md-8 col-lg-8 col-xs-12">
                <p className='fw-bold'>{user?.name}</p>
                <h2>{user?user.email:null}</h2>
                <div className='row row-cols-auto mb-1 mt-4'>
                    <div className='col'><p><b>{posts?.length}</b> posts</p></div>
                    <div className='col'><p><b>{user.followers?.length}</b> followers</p></div>
                    <div className='col'><p><b>{user.following?.length}</b> following</p></div>
                </div>
                {user?.followers?.includes(state._id)?
                <button
                onClick={()=>{
                appService.unfollowUser({
                    user_id:user._id
                }).then((result) => {
                    localStorage.setItem('user',JSON.stringify(result.updatedCurrentUser));
                    dispath({type:'user',payload:result.updatedCurrentUser});
                    setUser(result.updatedPartnerUser)
                }).catch((err) => {
                    // window.alert(err.message);
                });
            }}
            className="btn waves-effect waves-light mt-3 #1e88e5 blue darken-1 text-light">
                UnFollow
            </button>
                :<button
                onClick={()=>{
                appService.followUser({
                    user_id:user._id
                }).then((result) => {
                    localStorage.setItem('user',JSON.stringify(result.updatedCurrentUser));
                    dispath({type:'user',payload:result.updatedCurrentUser});
                    setUser(result.updatedPartnerUser)
                }).catch((err) => {
                    // window.alert(err.message);
                });
            }}
            className="btn waves-effect waves-light mt-3 #1e88e5 blue darken-1 text-light">
                Follow
            </button>
}
        
            </div>
        </div>
        <div className='row row-cols-xs-1 row-cols-lg-3 mt-5'>
            {posts.map((item,index)=>{
                return <img src={item.photo} className='col col-xs-12 mb-4 user-profile-image' />;
            })}
        </div>
        </div>
        </div>
    );
}

export default UserProfile;